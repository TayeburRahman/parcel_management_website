...
import { useDeleteParcelMutation, useGetParcelsQuery } from "@/redux/features/parcel/parcelApi";
...
const ParcelManagementPage = () => {
  ...
  const { data, isLoading, refetch } = useGetParcelsQuery({ page, query, date: startDate ? format(startDate, "yyyy-MM-dd") : "" });
  ...
  return (
    <PageContainer>
      ...
      <div className="overflow-auto md:h-[calc(100vh-175px)] h-[calc(100vh-240px)] scrl-hide rounded-md border border-gray-200">
        {isLoading ? <TableLoader /> : <ParcelTable paged={paged} handleDelete={handleDeleteClick} onAssignAgentClick={onAssignAgentClick} />}
      </div>
      <Pagination
